...
class Application(object):
    def __init__(self):
        self.message_string = ""
        self.green_lane_json = {}
        load_dotenv('.env')

        self.GREEN_LANE_CONFIG_FILE = os.getenv('GREEN_LANE_CONFIG_FILE')
        self.MEASURES_FILENAME = os.getenv('MEASURES_FILENAME')
        self.COMMODITY_CODES_FILENAME = os.getenv('COMMODITY_CODES_FILENAME')
    ...
    def get_filename(self):
        ...
        self.file_only = self.MEASURES_FILENAME + "_{dt}.csv".format(dt=self.SNAPSHOT_DATE)
        self.filename = os.path.join(self.dated_folder, self.file_only)

        self.file_only_comm_codes = self.COMMODITY_CODES_FILENAME + "_{dt}.csv".format(dt=self.SNAPSHOT_DATE)
        self.filename_comm_codes = os.path.join(self.dated_folder, self.file_only_comm_codes)
    ...
    def get_commodities(self):
        ...
        self.workbook_csv = open(self.filename, "w")
        self.workbook_csv_comm_codes = open(self.filename_comm_codes, "w")
        ...
        for field, column_width in (fields):
            self.workbook_csv_comm_codes.write(field + ",")
            col += 1
        ...
        for i in range(self.start, self.end):
            ...
            validity_start_date, validity_end_date, description, number_indents, leaf, significant_digits
            from utils.goods_nomenclature_export_kai(%s, %s, %s)
            order by 2, 3"""
            ...
            for row in rows:
                ...
                commodity.leaf = int(str(row[7]))
                commodity.significant_digits = row[8]
            ...
            self.get_green_lane_influences()
            self.extract_data()

        # Actions to be completed after the end of the last iteration
        self.start_timer("Saving green lane crunch file")
        self.workbook_csv.close()
        self.workbook_csv_comm_codes.close()
        with open('green_lane_json.json', 'w') as f:
    ...
    def get_footnotes(self, i):
        ...
        sql = """
        select f.measure_sid, f.footnote, f.validity_start_date, f.validity_end_date, f.goods_nomenclature_item_id
        from utils.materialized_measure_footnotes f, goods_nomenclatures gn, measure_types mt
        where f.validity_start_date <= %s
        and f.measure_type_id = mt.measure_type_id
        and f.goods_nomenclature_sid = gn.goods_nomenclature_sid
        and gn.validity_end_date is null
        and (f.validity_end_date is null or f.validity_end_date >= %s)
        and left(f.goods_nomenclature_item_id, 1) = %s
        and mt.measure_type_series_id != 'N'
        and mt.measure_type_id in %s
        and footnote > '99999'
        """
        ...
        for row in rows:
            ...
            self.measures_dict[footnote.measure_sid].footnotes.append(footnote)
    ...
    def get_condition_strings(self):
        for m in self.measures_dict:
            self.measures_dict[m].get_condition_string()
            ...
            if commodity.goods_nomenclature_item_id not in self.hidden_goods_nomenclatures:
                ...
                if commodity.leaf == 1:
                    for measure in commodity.measures:
                        if measure.measure_sid == 2865968:
                            if commodity.goods_nomenclature_item_id == "0303541010":
                                a = 1

                        if commodity.validity_end_date != "00000000":
                            cdate = commodity.validity_end_date[0:4] + "-" + commodity.validity_end_date[4:6] + "-" + commodity.validity_end_date[6:8]
                            if cdate < measure.validity_start_date:
                                show_measure = False
                            elif cdate < measure.validity_end_date or measure.validity_end_date == '':
                                show_measure = True
                                end_date = cdate
                            else:
                                show_measure = True
                                end_date = measure.validity_end_date
                        else:
                            end_date = measure.validity_end_date
                            show_measure = True
    ...
    def create_measure_duties(self):
        ...
        for measure in self.measures_dict:
            self.measures_dict[measure].create_measure_duties()
    ...
    def get_measures(self, iteration):
        ...
        order by m.goods_nomenclature_item_id, m.measure_type_id, m.geographical_area_id,
        m.ordernumber, m.additional_code_type_id, m.additional_code_id;"""
    ...
    def get_measure_components(self, iteration):
        ...
        from measure_components mc, utils.materialized_measures_real_end_dates m, measure_types mt
        ...
        for row in rows:
            ...
            self.measures_dict[measure_component.measure_sid].measure_components.append(measure_component)
    ...
    def get_measure_excluded_geographical_areas(self, iteration):
        ...
        for row in rows:
            ...
            self.measures_dict[measure_excluded_geographical_area.measure_sid].measure_excluded_geographical_areas.append(measure_excluded_geographical_area)
    ...
    def get_folders(self):
        ...
        # JSON for green lanes
        self.resources_folder = os.path.join(self.current_folder, "resources")
        self.json_folder = os.path.join(self.resources_folder, "json")
    ...
    def get_additional_codes_friendly(self):
        ...
        rows = d.run_query(sql)
        for row in rows:
            self.additional_codes_friendly[row[0]] = row[1].replace('"', '').replace('\n', ' ').replace('  ', ' ')
    ...
    def get_green_lane_influences(self):
        # Populate a variable with all their children
        for i in range(0, len(self.commodities)):
            my_commodity = self.commodities[i]
        ...
        # Move the commodities into a dict to speed up access
        commodities_dict = {}
        for commodity in self.commodities:
            commodities_dict[commodity.goods_nomenclature_sid] = commodity

        for commodity in commodities_dict:
            c = commodities_dict[commodity]
            if c.green_lane_eligible:
                for regulation in self.config_regulations:
                    for m in c.measures:
                        if m.measure_generating_regulation_id == regulation:
                            obj = {
                                "goods_nomenclature_item_id": c.goods_nomenclature_item_id,
                                "goods_nomenclature_sid": c.goods_nomenclature_sid,
                                "default_category": self.config_regulations[regulation]["default_category"],
                                "document_codes": self.config_regulations[regulation]["document_codes"],
                                "additional_codes": self.config_regulations[regulation]["additional_codes"]
                            }
                            if m.measure_type_id not in c.green_lane_data["measure_types"]:
                                c.green_lane_data["measure_types"][m.measure_type_id] = []
                            c.green_lane_data["measure_types"][m.measure_type_id].append(obj)

                    for child_sid in c.child_sids:
                        c2 = commodities_dict[child_sid]
                        for m in c2.measures:
                            if m.measure_generating_regulation_id == regulation:
                                obj = {
                                    "goods_nomenclature_item_id": c2.goods_nomenclature_item_id,
                                    "goods_nomenclature_sid": c2.goods_nomenclature_sid,
                                    "default_category": self.config_regulations[regulation]["default_category"],
                                    "document_codes": self.config_regulations[regulation]["document_codes"],
                                    "additional_codes": self.config_regulations[regulation]["additional_codes"]
                                }
        ...
        for commodity in commodities_dict:
            c = commodities_dict[commodity]
            if c.goods_nomenclature_item_id == "2709001000":
                a = 1
            if c.green_lane_eligible:
                if len(c.green_lane_data["measure_types"]) > 0:
                    obj = {
                        "goods_nomenclature_item_id": c.goods_nomenclature_item_id,
                        "productline_suffix": c.productline_suffix,
                        "green_lane_data": c.green_lane_data
                    }
                    self.green_lane_json[c.goods_nomenclature_sid] = obj
                    filename = "{sid}.json".format(sid=c.goods_nomenclature_sid)
                    filename = os.path.join(self.json_folder, filename)
                    with open(filename, 'w') as fx:
                        json.dump(obj, fx, indent=4)
        ...
        for commodity in self.commodities:
            ...
            self.workbook_csv_comm_codes.write(str(f.bool_to_integer(commodity.green_lane_eligible)))
            self.workbook_csv_comm_codes.write(",")

            self.workbook_csv_comm_codes.write(str(commodity.goods_nomenclature_sid))
            self.workbook_csv_comm_codes.write(",")

            self.workbook_csv_comm_codes.write(str(commodity.significant_digits))
            self.workbook_csv_comm_codes.write(",")
        ...
        for commodity in self.commodities:
            if commodity.leaf == 0:
                if (commodity.significant_digits >= 6 and commodity.productline_suffix == "80") or (commodity.significant_digits == 4 and commodity.goods_nomenclature_item_id[-2:] != "00"):
                    a = 1
        return
